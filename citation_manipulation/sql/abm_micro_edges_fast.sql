...
CREATE UNIQUE INDEX IF NOT EXISTS rolap.idx_aaf_orcid ON abm_author_filter(orcid);
...
JOIN rolap.works_doi_map wdm ON wdm.doi_norm = wrd.doi_norm         -- << Optimized join using pre-calculated map
JOIN work_authors         wa2 ON wa2.work_id = wdm.work_id
JOIN rolap.abm_citing_authors_count ca ON ca.work_id = we1.work_id
JOIN rolap.abm_cited_authors_count  za ON za.work_id = wdm.work_id
